...
exports.handler = async (event) => {
  const headersBase = { "Cache-Control": "no-store, no-cache, must-revalidate" };

  if (event.httpMethod !== "POST") {
    return { statusCode: 405, headers: headersBase, body: "Method Not Allowed" };
  }
  ...
  try {
    ...
    if (!imageUrl) {
      return {
        statusCode: 400,
        headers: headersBase,
        body: JSON.stringify({ error: "Falta imageUrl en el body", version: "v2-gemini3.6" }),
      };
    }
    ...
    if (!apiKey) {
      return {
        statusCode: 500,
        headers: headersBase,
        body: JSON.stringify({ error: "Falta GEMINI_API_KEY en el entorno", version: "v2-gemini3.6" }),
      };
    }
    ...
    if (!imgResponse.ok) {
      return {
        statusCode: 400,
        headers: headersBase,
        body: JSON.stringify({ error: "No se pudo descargar la imagen desde ese link", version: "v2-gemini3.6" }),
      };
    }
    ...
    const modeloUsado = "gemini-3.6-flash";
    const geminiResponse = await fetch(
      "https://generativelanguage.googleapis.com/v1beta/models/" + modeloUsado + ":generateContent?key=" +
        apiKey,
      {
        method: "POST",
    ...
    try {
      ...
      return {
        statusCode: 502,
        headers: headersBase,
        body: JSON.stringify({ error: "La IA no devolvió JSON válido", raw: rawText, version: "v2-gemini3.6" }),
      };
    }

    return { statusCode: 200, headers: headersBase, body: JSON.stringify(parsed) };
  } catch (err) {
    return { statusCode: 500, headers: headersBase, body: JSON.stringify({ error: err.message, version: "v2-gemini3.6" }) };
  }
};
